...
{
    // Test changing directory to a non-existent location.
    char *line = (char*) calloc(40, sizeof(char));
    // Provide an invalid directory path.
    strncpy(line, "cd /nonexistent_directory", 40);
    char **cmd = cmd_parse(line);
    int retval = change_dir(cmd);
...
{
    char **rval = cmd_parse("");
    //expect no tokens (i.e. the first element should be NULL).
    TEST_ASSERT_NULL(rval[0]);
    cmd_free(rval);
}


void test_cmd_parse_whitespace_only(void)
{
    char **rval = cmd_parse("     ");
    //expect no tokens (i.e. the first element should be NULL).
    TEST_ASSERT_NULL(rval[0]);
